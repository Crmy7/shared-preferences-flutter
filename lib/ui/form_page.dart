...
import 'profile_page.dart'; // Ajout de l'import manquant pour ProfilePage

class FormPage extends StatefulWidget {
  const FormPage({Key? key}); // Correction de la déclaration de la clé
...
class FormPageState extends State<FormPage> {
  ...
  List<String> genderList = ["Man", "Female"];

  String? programmingLanguageValue;
  List<String> programmingLanguageList = ["Dart", "Swift", "Kotlin"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          title: TextTitleWidget(title: "user page")), // Correction de la création du widget TextTitleWidget
          ...
          child: Column(
            children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      TextInfoIconWidget(
                          iconData: Icons.male,
                          information: user.genderToString()),
                      TextInfoIconWidget(
                          iconData: Icons.height,
                          information: user.heightToString()),
                      TextInfoIconWidget(
                          iconData: Icons.sports_basketball,
                          information: user.hobbiesToString()),
                      TextInfoIconWidget(
                          iconData: Icons.code,
                          information: user.favoriteProgrammingLanguage!),
                      Visibility(
                          visible: isSecretRevealed,
                          child: revealSecret()), // Correction de l'appel de la méthode revealSecret
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            Padding(
                              ...
                              child: imagePickerElevatedButton(
                                  iconData: Icons.browse_gallery,
                                  imageSource: ImageSource.gallery),
                            ),
                            imagePickerElevatedButton(
              ...
              customTextField(
                  ...
                  hint: "First name",
                  edgeInsets: const EdgeInsets.all(12.0)),
              customTextField(
              ...
              customTextField(
                  controller: secretEditingController,
                  hint: "Tell me a secret 🤫",
                  isSecret: true,
                  edgeInsets: const EdgeInsets.only(
                      bottom: 12.0, left: 12.0, right: 12.0)),
              ...
              hobbyCheckBoxList(),
              SectionTitleWidget(
                  width: width, label: "Coding is better with ..."),
              codeRadioListTile(),
              ElevatedButton( // Déplacement du bouton ElevatedButton à l'intérieur du SingleChildScrollView
                onPressed: () async {
                  await _saveUser();
                  if (mounted) {
                    Navigator.push(
    ...
    for (int i = 0; i < programmingLanguageList.length; i++) {
      Widget radio = Expanded(
        child: RadioListTile(
            ...
            onChanged: ((value) {
              setState(() {
                programmingLanguageValue = value as String?;
  ...
  updateUser() {
    setState(() {
      user = User(
          firstName: firstNameEditingController.text != user.firstName
              ? firstNameEditingController.text
              : user.firstName,
          lastName: lastNameEditingController.text != user.lastName
              ? lastNameEditingController.text
              : user.lastName,
          secret: secretEditingController.text != user.secret
              ? secretEditingController.text
              : user.secret,
          age: user.age,
          gender: user.gender,
          height: user.height,
  ...
  Widget imagePickerAvatar() {
    return Padding(
      ...
      child: CircleAvatar(
          ...
          child: CircleAvatar(
              radius: 28,
              backgroundImage: Image.file(file!, fit: BoxFit.cover).image)),
    );
  }
  ...
  Widget defaultAvatar() {
    return const Padding(
      ...
      child: CircleAvatar(
          radius: 30,
          backgroundColor: Colors.white,
          child: CircleAvatar(radius: 28, foregroundColor: Colors.blue)),
    );
  }

  Widget imagePickerElevatedButton(
      {required IconData iconData, required ImageSource imageSource}) {
